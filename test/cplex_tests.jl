...
function cplex_tests(compilation::Bool=false)
    ...
    if @isdefined CPLEX
        ...
        @testset "Solving storage_test with CPLEX" begin
            ...
            !compilation && @test isapprox(testqry[1,:val], 3840.94023817781; atol=TOL)
        ...
        @testset "Solving storage_transmission_test with CPLEX" begin
            ...
            !compilation && @test isapprox(testqry[1,:val], 9786.56626042587; atol=TOL)
            !compilation && @test isapprox(testqry[2,:val], 239.495064739257; atol=TOL)
            !compilation && @test isapprox(testqry[3,:val], 228.090641826873; atol=TOL)
            !compilation && @test isapprox(testqry[4,:val], 217.229273833058; atol=TOL)
            !compilation && @test isapprox(testqry[5,:val], 206.884786725342; atol=TOL)
            !compilation && @test isapprox(testqry[6,:val], 197.033272788862; atol=TOL)
            !compilation && @test isapprox(testqry[7,:val], 187.650830607229; atol=TOL)
            !compilation && @test isapprox(testqry[8,:val], 178.714951753762; atol=TOL)
            !compilation && @test isapprox(testqry[9,:val], 170.204748803116; atol=TOL)
            !compilation && @test isapprox(testqry[10,:val], 162.099761418328; atol=TOL)
            ...
            !compilation && @test isapprox(testqry[1,:val], 12672.8114711738; atol=TOL)
            !compilation && @test isapprox(testqry[2,:val], 2510.44588849556; atol=TOL)
...
end  # cplex_tests(compilation::Bool=false)
