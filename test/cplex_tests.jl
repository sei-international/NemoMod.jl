...
if @isdefined CPLEX
    ...
    @testset "Solving storage_test with CPLEX" begin
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(CPLEX.Optimizer), numprocs=1, varstosave="vproductionbytechnologyannual")
    ...
    @testset "Solving storage_transmission_test with CPLEX" begin
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(CPLEX.Optimizer), numprocs=1, varstosave="vproductionbytechnologyannual", calcyears=[2020,2025,2029])
        testqry = SQLite.DBInterface.execute(db, "select * from vproductionbytechnologyannual where t = 'gas' and y = 2025") |> DataFrame
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(CPLEX.Optimizer), numprocs=1, varstosave="vtotaldiscountedcost", calcyears=[2020,2025,2029])
        testqry = SQLite.DBInterface.execute(db, "select * from vtotaldiscountedcost") |> DataFrame
