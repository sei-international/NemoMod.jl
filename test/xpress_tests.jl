...
if @isdefined Xpress
    ...
    @testset "Solving storage_test with Xpress" begin
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = JuMP.Model(Xpress.Optimizer), restrictvars=false, numprocs=1)
        testqry = SQLite.DBInterface.execute(db, "select * from vtotaldiscountedcost") |> DataFrame
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(Xpress.Optimizer), numprocs=1, varstosave="vproductionbytechnologyannual")
    ...
    @testset "Solving storage_transmission_test with Xpress" begin
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(Xpress.Optimizer), numprocs=1, varstosave="vproductionbytechnologyannual", calcyears=[2020,2025,2029])
        testqry = SQLite.DBInterface.execute(db, "select * from vproductionbytechnologyannual where t = 'gas' and y = 2025") |> DataFrame
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = Model(Xpress.Optimizer), numprocs=1, varstosave="vtotaldiscountedcost", calcyears=[2020,2025,2029])
        testqry = SQLite.DBInterface.execute(db, "select * from vtotaldiscountedcost") |> DataFrame
