...
if @isdefined Xpress
    ...
    @testset "Solving storage_transmission_test with Xpress" begin
        ...
        if !compilation
            ...
            @test isapprox(testqry[1,:val], 9422.95248725373; atol=TOL)
        ...
        testnumber += 1
        @info "Running Xpress test $(testnumber) on storage_transmission_test.sqlite: TransmissionAnnualMaxCapacityInvestment."
    ...
    @testset "Solving subsidy_test with Xpress" begin
        testnumber = 0  # Counter used in @info messages
        ...
        # Test with default outputs
        testnumber += 1
        @info "Running Xpress test $(testnumber) on subsidy_test.sqlite: default outputs."
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Xpress.Optimizer) : direct_model(Xpress.Optimizer())),
        ...
        # Test with subsidies
        testnumber += 1
        @info "Running Xpress test $(testnumber) on subsidy_test.sqlite: subsidies."
        ...
        NemoMod.dropresulttables(db)
        testqry = SQLite.DBInterface.execute(db, "VACUUM")
    end  # "Solving subsidy_test with Xpress"
