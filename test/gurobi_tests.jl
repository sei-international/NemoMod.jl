...
function gurobi_tests(compilation::Bool=false)
    ...
    if @isdefined Gurobi
        ...
        @testset "Solving storage_test with Gurobi" begin
            ...
            !compilation && @test isapprox(testqry[1,:val], 3845.15703404259; atol=TOL)
            !compilation && @test isapprox(testqry[2,:val], 146.55227050539; atol=TOL)
            !compilation && @test isapprox(testqry[3,:val], 139.57362837926; atol=TOL)
            !compilation && @test isapprox(testqry[4,:val], 132.927266053843; atol=TOL)
            !compilation && @test isapprox(testqry[5,:val], 126.597396376304; atol=TOL)
            !compilation && @test isapprox(testqry[6,:val], 120.568948487497; atol=TOL)
            !compilation && @test isapprox(testqry[7,:val], 114.827569988092; atol=TOL)
            !compilation && @test isapprox(testqry[8,:val], 109.35959046485; atol=TOL)
            !compilation && @test isapprox(testqry[9,:val], 104.151990918904; atol=TOL)
            !compilation && @test isapprox(testqry[10,:val], 99.1923723037184; atol=TOL)
            ...
            NemoMod.calculatescenario(dbfile; jumpmodel = JuMP.Model(Gurobi.Optimizer), restrictvars=false)
            testqry = SQLite.DBInterface.execute(db, "select * from vtotaldiscountedcost") |> DataFrame
            ...
            NemoMod.calculatescenario(dbfile; jumpmodel = Model(Gurobi.Optimizer), varstosave="vproductionbytechnologyannual")
        ...
        @testset "Solving storage_transmission_test with Gurobi" begin
            ...
            !compilation && @test isapprox(testqry[1,:val], 9786.5662635601; atol=TOL)
            !compilation && @test isapprox(testqry[2,:val], 239.495064999922; atol=TOL)
            !compilation && @test isapprox(testqry[3,:val], 228.090642412206; atol=TOL)
            !compilation && @test isapprox(testqry[4,:val], 217.22918347106; atol=TOL)
            ...
            !compilation && @test isapprox(testqry[7,:val], 187.650735989392; atol=TOL)
            ...
            NemoMod.calculatescenario(dbfile; jumpmodel = Model(Gurobi.Optimizer), varstosave="vproductionbytechnologyannual", calcyears=[2020,2025,2029])
            testqry = SQLite.DBInterface.execute(db, "select * from vproductionbytechnologyannual where t = 'gas' and y = 2025") |> DataFrame
            ...
            NemoMod.calculatescenario(dbfile; jumpmodel = Model(Gurobi.Optimizer), varstosave="vtotaldiscountedcost", calcyears=[2020,2025,2029])
            testqry = SQLite.DBInterface.execute(db, "select * from vtotaldiscountedcost") |> DataFrame
            ...
            !compilation && @test isapprox(testqry[1,:val], 12672.81173526; atol=TOL)
...
end  # gurobi_tests(compilation::Bool=false)
