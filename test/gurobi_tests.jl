...
if @isdefined Gurobi
    ...
    @testset "Solving storage_test with Gurobi" begin
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: default outputs."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: optional outputs."
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())),
            varstosave =
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: restrictvars."
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())),
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: restrictvars and non time sliced fuels."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: storage net zero constraints."
        ...
        # Test with calcyears
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: calcyears."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: calcyears and non time sliced fuels."

        try
            SQLite.DBInterface.execute(db, "update FUEL set timesliced = 0 where val <> 'electricity'")

            NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())), varstosave = "vproductionbytechnology, vusebytechnology, vtotaldiscountedcost, vdemandannualnn, vdemandnn, vproductionbytechnology, vproductionnn, vrateofactivity, vrateofdemandnn, vrateofproduction, vrateofproductionbytechnologybymodenn, vrateofproductionbytechnologynn, vrateofproductionnn, vrateoftotalactivity, vrateofuse, vrateofusebytechnologybymodenn, vrateofusebytechnologynn, vrateofusenn, vtotalcapacityinreservemargin, vusebytechnology, vusenn", restrictvars=true, calcyears=[2020,2029], quiet = calculatescenario_quiet)
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_test.sqlite: minimum utilization."
    ...
    @testset "Solving storage_transmission_test with Gurobi" begin
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: non time sliced fuels."
        ...
        # Test MinimumUtilization
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: minimum utilization."
        ...
        # Test interest rates
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: interest rates."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: transshipment power flow."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: transmission line availability."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: minimum annual transmission between nodes."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: limited foresight optimization."
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: limited foresight optimization with non time sliced fuels."

        try
            SQLite.DBInterface.execute(db, "update FUEL set timesliced = 0 where val <> 'electricity'")

            NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(optimizer_with_attributes(Gurobi.Optimizer, "NumericFocus" => 2)) : direct_model(optimizer_with_attributes(Gurobi.Optimizer, "NumericFocus" => 2))), varstosave="vdemandannualnn, vdemandnn, vproductionbytechnology, vproductionnn, vrateofactivity, vrateofdemandnn, vrateofproduction, vrateofproductionbytechnologybymodenn, vrateofproductionbytechnologynn, vrateofproductionnn, vrateoftotalactivity, vrateofuse, vrateofusebytechnologybymodenn, vrateofusebytechnologynn, vrateofusenn, vtotalcapacityinreservemargin, vusebytechnology, vusenn, vdemandannualnodal, vdemandnodal, vgenerationannualnodal, vproductionannualnodal, vproductionnodal, vrateofactivitynodal, vrateofproductionbytechnologynodal, vrateofproductionnodal, vrateoftotalactivitynodal, vrateofusebytechnologynodal, vrateofusenodal, vregenerationannualnodal, vuseannualnodal, vusenodal, vnewcapacity, vtotalcapacityannual, vproductionbytechnologyannual, vusebytechnologyannual, vtotaldiscountedcost, vtransmissionbuilt, vtransmissionexists, vtransmissionbyline, vtransmissionannual", calcyears=[[2021,2022],[2025,2029]], quiet = calculatescenario_quiet)
        ...
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on storage_transmission_test.sqlite: TransmissionAnnualMaxCapacityInvestment."
    ...
    @testset "Solving ramp_test with Gurobi" begin
        ...
        # Test with default outputs
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on ramp_test.sqlite: default outputs."
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())),
            quiet = calculatescenario_quiet)
    ...
    @testset "Solving subsidy_test with Gurobi" begin
        testnumber = 0  # Counter used in @info messages
        ...
        # Test with default outputs
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on subsidy_test.sqlite: default outputs."
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())),
            quiet = calculatescenario_quiet, reportzeros=true)
        ...
        # Test with subsidies
        testnumber += 1
        @info "Running Gurobi test $(testnumber) on subsidy_test.sqlite: subsidies."

        try
            SQLite.DBInterface.execute(db, "insert into TechnologySubsidy select null, 1, 'wind', val, 3800.0 from year")

            NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Gurobi.Optimizer) : direct_model(Gurobi.Optimizer())),
                quiet = calculatescenario_quiet, reportzeros=true, varstosave="vproductionbytechnologyannual, vsubsidybyregion")
        ...
        NemoMod.dropresulttables(db)
        testqry = SQLite.DBInterface.execute(db, "VACUUM")
    end  # "Solving subsidy_test with Gurobi"
