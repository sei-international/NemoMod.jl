...
if @isdefined MosekTools
    ...
    @testset "Solving storage_test with Mosek" begin
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Mosek.Optimizer) : direct_model(Mosek.Optimizer())),
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Mosek.Optimizer) : direct_model(Mosek.Optimizer())), restrictvars=false, forcemip=true)
        ...
        # Test with calcyears
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Mosek.Optimizer) : direct_model(Mosek.Optimizer())), restrictvars=true, forcemip=true,
            calcyears=[2020,2029])
        ...
        NemoMod.calculatescenario(dbfile; jumpmodel = (reg_jumpmode ? Model(Mosek.Optimizer) : direct_model(Mosek.Optimizer())), varstosave="vproductionbytechnologyannual", forcemip=true)
