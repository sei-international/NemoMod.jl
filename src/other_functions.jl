...
function convertscenariounits(path::String; energy_multiplier::Float64 = 1.0, power_multiplier::Float64 = 1.0, 
    ...
    try
        ...
        # CapitalCost, FixedCost, TechnologySubsidy: cost/power
        for r in SQLite.DBInterface.execute(db,"select tbl_name from sqlite_master where type ='table' and name in ('CapitalCost', 'FixedCost', 'TechnologySubsidy')")
