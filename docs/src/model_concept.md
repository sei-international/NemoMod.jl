...
NEMO is a deterministic model, but its robust performance makes it practical to explore uncertainties in inputs, including in a large scenario ensemble analysis framework. It represents an energy system through a series of input parameters, calculated variables, constraints, and an objective function. A set of input parameters (and optionally custom constraints) for NEMO constitutes a *scenario*. You define parameters for a scenario by specifying them in a [NEMO scenario database](@ref scenario_db). Each scenario has its own database, and NEMO writes results to the database when the scenario is calculated.
