...
* **`calculatescenarioargs`** block *(arguments for [`calculatescenario`](@ref))*
  + **`jumpdirectmode`** - Indicates whether JuMP's direct mode should be used in the calculation (`true` or `false`). Direct mode bypasses some JuMP features that help to ensure compatibility with a wide range of solvers. These include reformulating (or bridging) the constraints, variables, and optimization objective that NEMO defines as needed to promote cross-solver compatibility. When direct mode is invoked, scenario calculation is generally faster and requires less memory. However, it can result in a solver error in some cases. For more information on direct mode, see the [JuMP documentation](https://jump.dev/JuMP.jl/stable/). If this option is set in a configuration file, it overrides the `jumpdirectmode` argument for `calculatescenario`.
  + **`jumpbridges`** - Indicates whether JuMP's bridging features should be used in the calculation (ignored if `jumpdirectmode` is `true`, as this implies bridging is disabled). Bridging involves reformulating the constraints, variables, and optimization objective that NEMO defines as needed to promote cross-solver compatibility. When bridging is disabled, scenario calculation is generally faster and requires less memory. However, it can result in a solver error in some cases. For more information on bridging, see the JuMP documentation. If this option is set in a configuration file, it overrides the `jumpbridges` argument for `calculatescenario`.
