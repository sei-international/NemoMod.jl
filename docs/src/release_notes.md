...
## Version 2.4
  ...
  * **Technology groups:** Implemented a new [dimension](@ref dimensions) in NEMO, [technology groups](@ref technologygroup). These are arbitrary groups of [technologies](@ref technology) that can be used in certain constraints. Technologies are mapped to technology groups in the new [`TTGroup`](@ref TTGroup) parameter.

  * **Technology subsidies:** Added support for modeling [technology](@ref technology) subsidies, which function like a discount on technology capital costs. A new parameter, [`TechnologySubsidy`](@ref TechnologySubsidy), defines the maximum permissible subsidy amount per unit of new (endogenously built) technology capacity. Total subsidies disbursed are tracked in two decision variables, [`vsubsidybytechnology`](@ref vsubsidybytechnology) (total subsidies for a technology in a [region](@ref region) and [year](@ref year)) and [`vsubsidybyregion`](@ref vsubsidybyregion) (sum of `vsubsidybytechnology` across technologies). You can limit the total subsidies disbursed at the technology, [technology group](@ref technologygroup), or region level (with the [`MaxSubsidyPerTechnology`](@ref MaxSubsidyPerTechnology), [`MaxSubsidyPerTechnologyGroup`](@ref MaxSubsidyPerTechnologyGroup), and [`MaxSubsidyPerRegion`](@ref MaxSubsidyPerRegion) parameters, respectively). If NEMO elects to apply a subsidy to new technology capacity, this reduces the capacity's [capital investment](@ref vcapitalinvestment) and [financing cost](@ref vfinancecost). These variables and downstream variables that depend on them, such as [salvage value](@ref vsalvagevalue) and [total discounted cost](@ref vtotaldiscountedcost), are net of subsidies.

  * **Maximum production share:** Added [`MaxShareProduction`](@ref MaxShareProduction), a parameter that specifies a [technology's](@ref technology) maximum share of production of a fuel in a given region and year (excluding production from storage).

  * **Capacity constraints by region group and technology group:** Provided eight new parameters that allow you to set [technology](@ref technology) capacity deployment limits by [region group](@ref regiongroup) or [technology group](@ref technologygroup): [`TotalAnnualMaxCapacityRG`](@ref TotalAnnualMaxCapacityRG) (maximum exogenous + endogenous capacity for a technology in a region group), [`TotalAnnualMaxCapacityTG`](@ref TotalAnnualMaxCapacityTG) (maximum exogenous + endogenous capacity for a technology group in a [region](@ref region)), [`TotalAnnualMaxCapacityInvestmentRG`](@ref TotalAnnualMaxCapacityInvestmentRG) (maximum addition of endogenous capacity for a technology in a region group), [`TotalAnnualMaxCapacityInvestmentTG`](@ref TotalAnnualMaxCapacityInvestmentTG) (maximum addition of endogenous capacity for a technology group in a region), [`TotalAnnualMinCapacityRG`](@ref TotalAnnualMinCapacityRG) (minimum exogenous + endogenous capacity for a technology in a region group), [`TotalAnnualMinCapacityTG`](@ref TotalAnnualMinCapacityTG) (minimum exogenous + endogenous capacity for a technology group in a region), [`TotalAnnualMinCapacityInvestmentRG`](@ref TotalAnnualMinCapacityInvestmentRG) (minimum addition of endogenous capacity for a technology in a region group), and [`TotalAnnualMinCapacityInvestmentTG`](@ref TotalAnnualMinCapacityInvestmentTG) (minimum addition of endogenous capacity for a technology group in a region).
